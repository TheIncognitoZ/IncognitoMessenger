...
//	}
	public List<Message> getMessage(String username)
	{
		...
		while(myRes.next())
		{
			m = new Message(myRes.getString("sender"), myRes.getString("content"), myRes.getString("receiver"), myRes.getString("publickey"), myRes.getString("sign")); 
			list.add(m);
		}
 		}
		catch(Exception e)
	...
	public Message putMessage(Message m)
	{
		...
		String query = "INSERT INTO "+m.getReceiver()+" (`sender`,`receiver`,`content`,`publickey`,`sign`) VALUES ('"+m.getAuthor()+"','"+m.getReceiver()+"','"+m.getContent()+"','"+m.getPublickey()+"','"+m.getSign()+"')";
